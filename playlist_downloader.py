...
if __name__ == '__main__':
    ...
    SPOTIFY_USERNAME = 'woseseltops'
    SPOTIFY_PLAYLIST_ID = '0abjz3cC0KSRxA4c2iIJZ3'
    SPOTIFY_CREDENTIAL_FILE_LOCATION = 'spotify_credentials.json'
    OUTPUT_FOLDER = 'Wessel wild\\'

    playlist = download_playlist(SPOTIFY_USERNAME,SPOTIFY_PLAYLIST_ID,SPOTIFY_CREDENTIAL_FILE_LOCATION)

    for artist, song_title in playlist:

        print('Getting link for '+artist+' - '+song_title)
        youtube_url = find_youtube_url(artist,song_title)

        print('Downloading '+artist+' - '+song_title)
        try:
            save_youtube_video(youtube_url,OUTPUT_FOLDER+artist+' - '+song_title+'.mp3')
        except:
